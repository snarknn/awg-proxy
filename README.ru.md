...
| `dns_via_tunnel` | `true` | Маршрутизировать DNS через туннели (anti-hijack). Поставьте `false` для обычного DNS через default route (например, для LAN резолверов) |
...
- **DNS anti-hijack (включен по умолчанию):** DNS-запросы от microsocks маршрутизируются через туннели, чтобы предотвратить их перехват провайдером/ТСПУ. Каждый неймсервер назначается по кругу на один из запущенных туннелей через `ip rule add to <ns> lookup <table>` + iptables SNAT на IP AWG-интерфейса.
- Установите `dns_via_tunnel: false` в `global:`, чтобы отключить это и пустить DNS через default route (например, для LAN-резолверов вроде Pi-hole/AdGuard).
- Для полной изоляции DNS по туннелям см. раздел «Устранение проблем».
...
- **Дублирование внутреннего IP (подсеть Amnezia по умолчанию 10.8.1.0/24)**
  - Установщик AmneziaWG по умолчанию всегда использует подсеть `10.8.1.0/24` и выдаёт IP последовательно (`.2`, `.3`, `.4`...).
  - Если несколько серверов стоят с дефолтами, туннели могут получить одинаковые внутренние IP (например, два туннеля с `10.8.1.2`).
  - **Поведение:** Контейнер обнаружает это при старте, выводит предупреждение и **не включает более поздние туннели с повторным адресом**, поднимая остальные.
  - **Исправление:** На сервере (Amnezia Web UI или CLI) измените `AllowedIPs` пира на свободный IP в той же подсети (например, `10.8.1.50/32`). В клиентском `.conf` измените `Address` на соответствующий (например, `Address = 10.8.1.50/24`). Перезапустите контейнер.
...
- Нужна изоляция DNS по туннелям
  ...
  - **DNS anti-hijack включен по умолчанию** (`dns_via_tunnel: true`). DNS-запросы маршрутизируются через туннели для защиты от отравления.
  - Установите `dns_via_tunnel: false` в `global:`, чтобы отключить и использовать обычный DNS через default route (например, для LAN-резолверов вроде Pi-hole/AdGuard).
  - Для полной изоляции DNS используйте network namespaces (нужна capability `SYS_ADMIN`).
